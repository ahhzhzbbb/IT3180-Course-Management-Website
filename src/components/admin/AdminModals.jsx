      ...
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={onSave} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          ...
          <div><label>Password {isEdit && <small>(Blank to keep)</small>}</label>
            <input type="password" value={form.password} required={!isEdit} onChange={e => setForm({ ...form, password: e.target.value })} />
          </div>
          {!isEdit && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
              <div><label>Phone</label><input required value={form.phoneNumber} onChange={e => setForm({ ...form, phoneNumber: e.target.value })} /></div>
              <div><label>Birth Date</label><input type="date" required value={form.birth} onChange={e => setForm({ ...form, birth: e.target.value })} /></div>
            </div>
          )}
          {!isEdit && (
          ...
          <div style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
            ...
            <button type="button" onClick={onClose} style={{ flex: 1, background: '#f3f4f6', border: '1px solid #e5e7eb', color: '#374151', borderRadius: '8px', cursor: 'pointer' }}>Cancel</button>
          </div>
        </form>
      </div>
    </div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={onSave} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
          <div><label>Course Title</label><input required value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></div>
